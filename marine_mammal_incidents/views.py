...
SEX_MAP = {"F": "Female", "M": "Male", "U": "Unknown", "f": "Female", "m": "Male", "u": "Unknown"}
    ...
    if request.method == "POST":
        ...
        if form.is_valid() and formset.is_valid():
            try:
                ...
                response_data = {"status": "success", "message": "Incident saved successfully"}
            ...
            errors = {"status": "error", "errors": {"form_errors": form.errors, "formset_errors": [f.errors for f in formset]}}
...
def get_locations(request):
    ...
    locations = Incident.objects.filter(incident_date__range=[start_date, end_date]).values_list("location_name", flat=True).distinct()
            ...
            for row in ws.iter_rows(min_row=2):
                try:
                    ...
                    duplicate_query = Incident.objects.filter(
                        species=species, incident_date=incident_date, incident_time=incident_time, incident_type=incident_type
                    )
                    if geo_location:
                    ...
                    if duplicate_query.exists():
                        ...
                        failed_rows.append(
                            {"row_number": row[0].row, "data": row_data, "error": "Duplicate incident - already exists in database"}
                        )
                    ...
                    row_data = [cell.value for cell in row]
                    failed_rows.append({"row_number": row[0].row, "data": row_data, "error": str(e)})
