    ...
    def get_queryset(self):
        ...
        if "filter" in self.request.GET and self.request.GET["filter"] == "no_observation_id":
            has_dataentry_no_observation_id = Exists(
                TrtDataEntry.objects.filter(entry_batch_id=OuterRef("pk"), observation_id__isnull=True)
            )
        ...
        last_place_code_subquery = Subquery(
            TrtDataEntry.objects.filter(entry_batch_id=OuterRef("pk")).order_by("-data_entry_id").values("place_code")[:1]
        )
        ...
        queryset = queryset.annotate(
            ...
            do_not_process_count=Count("trtdataentry", filter=Q(trtdataentry__do_not_process=True)),
        )
...
class EntryBatchDetailView(LoginRequiredMixin, FormMixin, ListView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        has_permission = TrtEntryBatchOrganisation.objects.filter(trtentrybatch_id=batch_id, organisation__in=org_codes).exists()

        if not has_permission:
            raise PermissionDenied("You do not have permission to view this batch")

        if not (user.groups.filter(name__in=["WAMTRAM2_VOLUNTEER", "WAMTRAM2_TEAM_LEADER", "WAMTRAM2_STAFF"]).exists()):
            raise PermissionDenied("You do not have permission to view this batch")
    ...
    def get(self, request, *args, **kwargs):
        if "batch_id" not in kwargs:
            new_batch = TrtEntryBatches.objects.create(pr_date_convention=False, entry_date=timezone.now().date())
    ...
    def get_queryset(self):
        ...
        if filter_value == "needs_review":
            ...
            queryset = queryset.filter(entry_batch_id=batch_id, error_message__isnull=False).exclude(
                error_message__in=["None", "Observation added to database"]
            )
        else:
    ...
    def get_context_data(self, **kwargs):
        ...
        context["form"] = TrtEntryBatchesForm(instance=batch, initial=initial)
        ...
        context["do_not_process_entries"] = TrtDataEntry.objects.filter(entry_batch_id=batch.entry_batch_id, do_not_process=True).order_by(
            "-data_entry_id"
        )

        # Add entries with do_not_process = False to the context
        context["process_entries"] = TrtDataEntry.objects.filter(entry_batch_id=batch.entry_batch_id, do_not_process=False).order_by(
            "-data_entry_id"
        )
    ...
    def post(self, request, *args, **kwargs):
        try:
            ...
            try:
                ...
                return JsonResponse({"success": False, "error": "Invalid team leader selected"})
            ...
            if template_id:
                try:
                    ...
                    return JsonResponse({"success": False, "error": "Invalid template selected"})

            if night_end < night_start:
                return JsonResponse({"success": False, "error": "End night must be greater than or equal to start night"})
            ...
            for night in range(night_start, night_end + 1):
                try:
                    ...
                    for org in request.user.organisations.all():
                        TrtEntryBatchOrganisation.objects.create(trtentrybatch=batch, organisation=org.code)
                    ...
                    return JsonResponse({"success": False, "error": f"Error creating batch {night}: {str(e)}"})

            return JsonResponse({"success": True, "batches": batches_created})
...
class MultiBatchEntryListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        if search:
            queryset = queryset.filter(Q(data_entry_id__icontains=search) | Q(error_message__icontains=search))
...
class TrtDataEntryFormView(LoginRequiredMixin, FormView):
    ...
    def get_initial(self):
        ...
        initial["is_recapture"] = bool(tag_id and tag_type)
    ...
    def form_valid(self, form):
        ...
        if form.instance.do_not_process:
            message = (
                f"Entry created successfully and will be reviewed later. Please write the Entry ID: {entry.data_entry_id} on the data sheet"
            )
            message_tag = "warning"
        else:
        ...
        if self.request.headers.get("x-requested-with") == "XMLHttpRequest":
            return JsonResponse({"success": True, "redirect_url": success_url, "message": message, "message_tag": message_tag})
        else:
    ...
    def form_invalid(self, form):
        ...
        if self.request.headers.get("x-requested-with") == "XMLHttpRequest":
            return JsonResponse({"success": False, "errors": form.errors, "message": error_message})
        else:
...
class DeleteBatchView(LoginRequiredMixin, View):
    def dispatch(self, request, *args, **kwargs):
        # FIXME: Permission check
        if not (request.user.groups.filter(name="WAMTRAM2_STAFF").exists() or request.user.is_superuser):
            return HttpResponseForbidden("You do not have permission to view this record")
        return super().dispatch(request, *args, **kwargs)
    ...
    def get(self, request, *args, **kwargs):
        ...
        if tag_id and tag_type and not turtle:
            new_tag_entry = (
                ...
                .select_related("entry_batch", "place_code", "species_code")
                .order_by("-entry_batch__entry_date")
                .first()
            )
            ...
            if not create_and_review:
                ...
                if not turtle:
                    existing_turtle_entry = (
                        ...
                        .select_related("turtle_id", "entry_batch", "place_code", "species_code")
                        .order_by("-entry_batch__entry_date")
                        .first()
                    )
...
class TemplateManageView(LoginRequiredMixin, FormView):
    ...
    def form_valid(self, form):
        ...
        if self.request.headers.get("x-requested-with") == "XMLHttpRequest":
            return JsonResponse({"message": "Template created successfully", "status": "success"})
    ...
    def form_invalid(self, form):
        ...
        if self.request.headers.get("x-requested-with") == "XMLHttpRequest":
            return JsonResponse({"message": "Form submission failed", "errors": errors, "status": "error"}, status=400)
            ...
            try:
                ...
                if tag_obj:
                    if tag_obj.turtle_id != turtle_id:
                        ...
                        return JsonResponse(
                            {"valid": is_valid, "wrong_side": wrong_side, "other_turtle_id": None, "status": tag_obj.tag_status.description}
                        )
...
def search_persons(request):
    ...
    if len(query_parts) == 1:
        persons = TrtPersons.objects.filter(Q(first_name__icontains=query_parts[0]) | Q(surname__icontains=query_parts[0])).values(
            "person_id", "first_name", "surname"
        )
    elif len(query_parts) >= 2:
        persons = TrtPersons.objects.filter(Q(first_name__icontains=query_parts[0]) & Q(surname__icontains=query_parts[1])).values(
            "person_id", "first_name", "surname"
        )
...
class ExportDataView(LoginRequiredMixin, View):
    ...
    def get_locations(self, request):
        ...
        if entry_type == "processed":
            ...
            if not user.is_superuser:
                related_batch_ids = TrtEntryBatchOrganisation.objects.filter(organisation__in=user_orgs).values_list(
                    "trtentrybatch_id", flat=True
                )
        ...
        locations = TrtLocations.get_ordered_locations().filter(location_code__in=locations.values_list("location_code", flat=True))

        location_list = [{"value": location.location_code, "label": location.location_name} for location in locations]
    ...
    def get_species(self, request):
        ...
        species_list = [{"value": specie.species_code, "label": specie.common_name} for specie in species]
    ...
    def export_data(self, request):
        try:
            ...
            batch_orgs = TrtEntryBatchOrganisation.objects.filter(trtentrybatch_id__in=batch_ids).values("trtentrybatch_id", "organisation")
...
class DudTagManageView(LoginRequiredMixin, View):
    ...
    def get(self, request):
        # Get flipper tag IDs
        flipper_tag_ids = set()
        flipper_entries = TrtDataEntry.objects.filter(Q(dud_flipper_tag__isnull=False) | Q(dud_flipper_tag_2__isnull=False))
        ...
        pit_tag_ids = set()
        pit_entries = TrtDataEntry.objects.filter(Q(dud_pit_tag__isnull=False) | Q(dud_pit_tag_2__isnull=False))
...
class BatchesCurationView(LoginRequiredMixin, PaginateMixin, ListView):
    ...
    def get_queryset(self):
        ...
        if user.is_superuser:
            ...
            for org in user_organisations:
                related_batch_ids = TrtEntryBatchOrganisation.objects.filter(organisation=org.code).values_list(
                    "trtentrybatch_id", flat=True
                )

            queryset = TrtEntryBatches.objects.filter(entry_batch_id__in=related_batch_ids).order_by("-entry_batch_id")

        last_place_code_subquery = Subquery(
            TrtDataEntry.objects.filter(entry_batch_id=OuterRef("pk")).order_by("-data_entry_id").values("place_code__place_name")[:1]
        )
        ...
        if user.is_superuser:
            queryset = queryset.annotate(processed_count=Count("trtdataentry", filter=Q(trtdataentry__observation_id__isnull=False)))
        ...
        if location and place and year:
            queryset = queryset.filter(location__location_code=location, place__place_code=place, year=year)
        elif location and year:
            queryset = queryset.filter(location__location_code=location, year=year)
        elif location:
    ...
    def get_context_data(self, **kwargs):
        ...
        places_data = [
            {"place_code": place.place_code, "place_name": place.place_name, "full_name": place.get_full_name()} for place in places
        ]
    ...
    def get(self, request):
        ...
        context = {
            ...
            "places_json": json.dumps(
                [{"place_code": p.place_code, "place_name": p.place_name, "full_name": p.get_full_name()} for p in TrtPlaces.objects.all()]
            ),
        }
        return render(request, self.template_name, context)
    ...
    def post(self, request):
        try:
            ...
            if data.get("entered_person_id"):
                try:
                    entered_person = TrtPersons.objects.get(person_id=int(data.get("entered_person_id")))
            ...
            if data.get("template_id"):
                try:
                    template = Template.objects.get(template_id=int(data.get("template_id")))
...
def search_templates(request):
    query = request.GET.get("q", "")
    if len(query) >= 2:
        templates = Template.objects.filter(Q(name__icontains=query))[:10]
...
class AddPersonView(LoginRequiredMixin, FormView):
    ...
    def form_valid(self, form):
        ...
        if self.request.headers.get("x-requested-with") == "XMLHttpRequest":
            return JsonResponse({"message": "Person added!", "status": "success"})
        ...
        if file:
            try:
                ...
                for _, row in df.iterrows():
                    ...
                    new_entries.append(
                        TrtPersons(first_name=row["first_name"], surname=row["surname"], email=email, recorder=row.get("recorder", False))
                    )
                ...
                if request.headers.get("x-requested-with") == "XMLHttpRequest":
                    response_data = {"message": f"{added_count} people added!", "status": "success"}
                    if skipped_count > 0:
...
class AvailableBatchesView(LoginRequiredMixin, View):
    def get(self, request):
        if request.user.is_superuser:
            ...
            batches = TrtEntryBatches.objects.filter(batch_organisations__organisation__in=[org.code for org in user_orgs])

        current_batch_id = request.GET.get("current_batch_id")

        batches = batches.exclude(entry_batch_id=current_batch_id).distinct()

        return JsonResponse(
            [{"id": batch.entry_batch_id, "code": batch.batches_code, "comment": batch.comments} for batch in batches], safe=False
        )
...
class BatchInfoView(LoginRequiredMixin, View):
    def get(self, request, batch_id):
        try:
            ...
            batch = TrtEntryBatches.objects.get(entry_batch_id=batch_id, batch_organisations__organisation__in=user_org_codes)
            return JsonResponse({"code": batch.batches_code, "comment": batch.comments})
        except TrtEntryBatches.DoesNotExist:
...
class MoveEntryView(LoginRequiredMixin, View):
    def post(self, request):
        ...
        try:
            ...
            entry = TrtDataEntry.objects.select_related("entry_batch").get(data_entry_id=entry_id)
            current_batch = entry.entry_batch

            if not current_batch.batch_organisations.filter(organisation__in=user_org_codes).exists():
                raise PermissionDenied("No permission to operate on this entry")

            target_batch = TrtEntryBatches.objects.get(entry_batch_id=target_batch_id, batch_organisations__organisation__in=user_org_codes)

            entry.entry_batch = target_batch
            entry.save()

            return JsonResponse({"success": True, "message": f"Successfully moved entry to batch {target_batch.batches_code}"})
...
class PersonManageView(LoginRequiredMixin, UserPassesTestMixin, PaginateMixin, ListView):
    ...
    def get_queryset(self):
        ...
        if search_term:
            ...
            if len(search_terms) == 1:
                term = search_terms[0]
                query = Q(first_name__icontains=term) | Q(surname__icontains=term) | Q(email__icontains=term)
                ...
                query = (
                    ...
                    | Q(first_name__icontains=search_terms[0], surname__icontains=" ".join(search_terms[1:]))
                    | reduce(
                        operator.or_,
                        (Q(first_name__icontains=term) | Q(surname__icontains=term) | Q(email__icontains=term) for term in search_terms),
                    )
                )

            queryset = queryset.filter(query)

        return queryset.prefetch_related("measurer_person", "tagger_person", "entered_by_person")
    ...
    def handle_merge(self):
        ...
        try:
            ...
            models_to_update = [
                ("TrtDataEntry", ["measured_by_id", "recorded_by_id", "tagged_by_id", "entered_by_id", "measured_recorded_by_id"]),
                (
                    "TrtObservations",
                    ["reporter_person", "entered_by_person", "tagger_person", "measurer_reporter_person", "measurer_person"],
                ),
            ]
            ...
            secondary.delete()
            messages.success(self.request, f"Successfully merged {secondary_info} into {primary.first_name} {primary.surname}")
...
class TagRegisterView(LoginRequiredMixin, FormView):
    ...
    def form_invalid(self, form):
        ...
        return JsonResponse({"success": False, "error": "Invalid form data: " + "; ".join(errors)})
...
class FlipperTagsListView(LoginRequiredMixin, UserPassesTestMixin, PaginateMixin, ListView):
    ...
    def get_queryset(self):
        queryset = super().get_queryset().select_related("turtle", "tag_status", "custodian_person")
...
class TransferObservationsByTagView(LoginRequiredMixin, View):
    ...
    def get_context_data(self):
        ...
        return {"page_title": "Transfer Observations - " + settings.SITE_TITLE}
    ...
    def get_turtle_info(self, turtle_id):
        ...
        try:
            ...
            return {"success": False, "error": f"Turtle {turtle_id} not found"}
    ...
    def get_observations(self, tag_id):
        ...
        observations = (
            TrtObservations.objects.filter(trtrecordedtags__tag_id=tag_id)
            .select_related("turtle")
            .values("observation_id", "observation_date", "turtle_id", "place_code", "comments")
    ...
    def post(self, request):
        ...
        if request.headers.get("X-Requested-With") == "FetchObservations":
            ...
            return JsonResponse({"success": True, "observations": observations})
        ...
        try:
            ...
            with connections["wamtram2"].cursor() as cursor:
                ...
                if return_value == 0:
                    return JsonResponse({"success": True, "message": error_message})
                else:
...
class NestingSeasonListView(LoginRequiredMixin, UserPassesTestMixin, PaginateMixin, ListView):
    ...
    def get_queryset(self):
        ...
        search = self.request.GET.get("search")
        if search:
            queryset = queryset.filter(Q(nesting_season__icontains=search))
...
class BatchCurationView(LoginRequiredMixin, SuperUserRequiredMixin, PaginateMixin, ListView):
    ...
    def get_queryset(self):
        ...
        queryset = queryset.select_related("entered_person_id", "template")

        queryset = queryset.annotate(
            entry_count=Count("trtdataentry"), flagged_count=Count("trtdataentry", filter=Q(trtdataentry__do_not_process=True))
        )
...
class EntryCurationView(LoginRequiredMixin, SuperUserRequiredMixin, PaginateMixin, ListView):
    ...
    def get_queryset(self):
        ...
        if search:
            queryset = queryset.filter(
                Q(comments__icontains=search) | Q(turtle_comments__icontains=search) | Q(species_code__code__icontains=search)
            )
        if not queryset.exists():
    ...
    def get_context_data(self, **kwargs):
        ...
        field_groups = {
            ...
            "Status and Activities": ["alive", "activity_code", "nesting", "interrupted", "identification_confidence"],
            "Flipper Tags": [
                "flipper_tag_check",
                "recapture_left_tag_id",
                "recapture_left_tag_state",
                ...
                "recapture_left_tag_state",
                "recapture_left_tag_barnaclesrecapture_left_tag_id_2",
                "recapture_left_tag_state_2",
            ...
            "Dud Tags": ["dud_flipper_tag", "dud_flipper_tag_2", "dud_pit_tag", "dud_pit_tag_2"],
            "Other Tags": ["other_tags", "other_tags_identification_typeidentifer", "identification_type"],
            "Tag Scars": [
                "scar_check",
                "scars_left",
                "scars_right",
            ...
            "Tissue Samples": ["tissue_type_1", "sample_label_1", "tissue_type_2", "sample_label_2"],
            "Nesting Data": ["egg_count", "egg_count_method", "clutch_completed"],
            "Personnel": [
                "entered_by_id",
                "measured_by_id",
                "recorded_by_id",
            ...
            "Comments": ["turtle_comments", "comment_fromrecordedtagstable", "error_number"],
        }
        ...
        if request.headers.get("X-Requested-With") == "XMLHttpRequest":
            ...
            # Handle batch edit
                ...
                try:
                    ...
                    return JsonResponse({"status": "success", "message": f"Successfully updated {entries.count()} entries"})
    ...
    def get_context_data(self, **kwargs):
        ...
        try:
            context.update(
                {
                    "tag_states_choices": [
                        {"tag_state": state.tag_state, "description": state.description} for state in TrtTagStates.objects.all()
                    ],
                    "pit_tag_state_choices": [
                        {"pit_tag_state": state.pit_tag_state, "description": state.description} for state in TrtPitTagStates.objects.all()
                    ],
                    "measurement_type_choices": [
                        {"measurement_type": type.measurement_type, "description": type.description}
                        for type in TrtMeasurementTypes.objects.all()
                    ],
                    "body_parts_choices": [
                        {"body_part": part.body_part, "description": part.description} for part in TrtBodyParts.objects.all()
                    ],
                    "damage_codes_choices": [
                        {"damage_code": code.damage_code, "description": code.description} for code in TrtDamageCodes.objects.all()
                    ],
                    "damage_cause_choices": [
                        {"damage_cause_code": cause.damage_cause_code, "description": cause.description}
...
class ObservationDataView(LoginRequiredMixin, SuperUserRequiredMixin, View):
    ...
    def post(self, request, observation_id=None):
        try:
            ...
            if "observation_date" in basic_info:
                try:
                    datetime_obj = datetime.strptime(basic_info["observation_date"], "%Y-%m-%dT%H:%M")
            ...
            return JsonResponse({"status": "error", "message": str(e)}, status=400 if isinstance(e, ValidationError) else 500)
        ...
        try:
            if observation_id:
                ...
                try:
                    ...
                    return JsonResponse({"status": "error", "message": f"Error getting observation data: {str(e)}"}, status=500)
            else:
            ...
            return JsonResponse({"status": "error", "message": f"Observation {observation_id} not found"}, status=404)
        except Exception as e:
    ...
    def _get_observation_data(self, observation):
        ...
        other_tags_data = {
            ...
            "identification_type": observation.other_tags_identification_type.identification_type
            if observation.other_tags_identification_type
            else None,
            "identification_type_description": observation.other_tags_identification_type.description
            if observation.other_tags_identification_type
            else None,
        }
        ...
        tag_info = {"recorded_tags": recorded_tags, "recorded_pit_tags": recorded_pit_tags}
        ...
        body_parts = [{"body_part": part.body_part, "description": part.description} for part in TrtBodyParts.objects.all()]

        damage_codes = [{"damage_code": code.damage_code, "description": code.description} for code in TrtDamageCodes.objects.all()]
        ...
        return {
            "basic_info": {
                ...
                "place_description": observation.place_description or "",
                "action_taken": observation.action_taken or "",
                "comments": observation.comments or "",
            },
            "tag_info": tag_info,
    ...
    def _filter_observations(self, request):
        ...
        if search_term:
            ...
            for part in tag_parts:
                q_objects |= Q(trtrecordedtags__tag_id__tag_id__exact=part) | Q(trtrecordedpittags__pittag_id__pittag_id__exact=part)
        ...
        observations = (
            observations.select_related("place_code")
            .prefetch_related("trtrecordedtags_set", "trtrecordedpittags_set")
            .distinct()
            .order_by("-observation_date", "-observation_time")
        )
...
class SaveObservationView(LoginRequiredMixin, SuperUserRequiredMixin, View):
    ...
    def post(self, request, observation_id=None):
        try:
            ...
            if not turtle_id:
                return JsonResponse({"status": "error", "message": "Turtle ID is required"}, status=400)

            try:
                turtle = TrtTurtles.objects.get(turtle_id=turtle_id)
                observation.turtle = turtle
            except TrtTurtles.DoesNotExist:
                return JsonResponse({"status": "error", "message": f"Turtle with ID {turtle_id} not found"}, status=400)
            ...
            if not observation.observation_date:
                return JsonResponse({"status": "error", "message": "Observation date is required"}, status=400)
        ...
        try:
            if observation.nesting and observation.nesting.code == "Y":
                previous_observations = TrtObservations.objects.filter(
                    turtle_id=observation.turtle_id, observation_date__lt=observation.observation_date
                ).exists()
...
class RecordedTagsUpdateView(LoginRequiredMixin, SuperUserRequiredMixin, View):
    def post(self, request, *args, **kwargs):
        try:
            ...
            # Handle deleted tags
            if deleted_tags:
                TrtRecordedTags.objects.filter(observation_id=observation, tag_id__tag_id__in=deleted_tags).delete()
            ...
            for tag_data in recorded_tags:
                ...
                tag, _ = TrtTags.objects.get_or_create(tag_id=tag_id)
            ...
            return JsonResponse({"status": "error", "message": "Invalid tag state"}, status=400)
        except Exception as e:
            return JsonResponse({"status": "error", "message": str(e)}, status=500)
...
class RecordedPitTagsUpdateView(LoginRequiredMixin, SuperUserRequiredMixin, View):
    def post(self, request, *args, **kwargs):
        try:
            ...
            # Handle deleted tags
            if deleted_tags:
                TrtRecordedPitTags.objects.filter(observation_id=observation, pittag_id__pittag_id__in=deleted_tags).delete()
            ...
            for tag_data in recorded_tags:
                ...
                if tag_data.get("pit_tag_state"):
                    tag_state = TrtPitTagStates.objects.get(pit_tag_state=tag_data.get("pit_tag_state"))
            ...
            return JsonResponse({"status": "success", "message": "PIT tags updated successfully"})
        ...
        try:
            ...
            if deleted_identifications:
                TrtRecordedIdentification.objects.filter(
                    observation_id=observation_id, recorded_identification_id__in=deleted_identifications
                ).delete()
            ...
            for identification_data in recorded_identifications:
                ...
                id_type = TrtIdentificationTypes.objects.get(identification_type=identification_type)
            ...
            return JsonResponse({"status": "success", "message": "Identifications updated successfully"})

        except TrtObservations.DoesNotExist:
            return JsonResponse({"status": "error", "message": "Observation not found"}, status=404)
        except TrtIdentificationTypes.DoesNotExist:
            return JsonResponse({"status": "error", "message": "Invalid identification type"}, status=400)
        except Exception as e:
            return JsonResponse({"status": "error", "message": str(e)}, status=500)
    ...
    def post(self, request, *args, **kwargs):
        try:
            ...
            if deleted_measurements:
                TrtMeasurements.objects.filter(observation_id=observation_id, id__in=deleted_measurements).delete()
            ...
            for measurement_data in recorded_measurements:
                ...
                meas_type = TrtMeasurementTypes.objects.get(measurement_type=measurement_type)
            ...
            return JsonResponse({"status": "success", "message": "Measurements updated successfully"})

        except TrtObservations.DoesNotExist:
            return JsonResponse({"status": "error", "message": "Observation not found"}, status=404)
        except TrtMeasurementTypes.DoesNotExist:
            return JsonResponse({"status": "error", "message": "Invalid measurement type"}, status=400)
        except Exception as e:
            return JsonResponse({"status": "error", "message": str(e)}, status=500)
...
class RecordedDamageUpdateView(LoginRequiredMixin, SuperUserRequiredMixin, View):
    def post(self, request, *args, **kwargs):
        try:
            ...
            if deleted_damage:
                TrtDamage.objects.filter(observation=observation, body_part_id__in=deleted_damage).delete()
            ...
            for damage_data in recorded_damage:
                ...
                TrtDamage.objects.filter(observation=observation, body_part_id=body_part).delete()

                # Create new record
                TrtDamage.objects.create(observation=observation, **damage_fields)

            return JsonResponse({"status": "success", "message": "Damage records updated successfully"})
...
class RecordedScarsUpdateView(LoginRequiredMixin, SuperUserRequiredMixin, View):
    def post(self, request, *args, **kwargs):
        try:
            ...
            return JsonResponse({"status": "success", "message": "Scars updated successfully"})
...
class TurtleManagementView(LoginRequiredMixin, SuperUserRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["tag_statuses"] = [
            {"tag_status": status.tag_status, "description": status.description} for status in TrtTagStatus.objects.all()
        ]
        context["pit_tag_statuses"] = [
            {"pit_tag_status": status.pit_tag_status, "description": status.description} for status in TrtPitTagStatus.objects.all()
        ]
        ...
        context["tissue_types"] = [
            {"tissue_type": type.tissue_type, "description": type.description} for type in TrtTissueTypes.objects.all()
        ]

        context["document_types"] = [
            {"document_type": type.document_type, "description": type.description} for type in TrtDocumentTypes.objects.all()
        ]
            ...
            try:
                ...
                with transaction.atomic():
                    try:
                        ...
                        return JsonResponse({"status": "error", "message": "Turtle not found"}, status=404)
                    ...
                    if "basic" in data:
                        ...
                        try:
                            ...
                            return JsonResponse({"status": "success", "message": "Turtle information updated successfully"})

                        except (
                            TrtSpecies.DoesNotExist,
                            TrtLocations.DoesNotExist,
                            TrtTurtleStatus.DoesNotExist,
                            TrtCauseOfDeath.DoesNotExist,
                        ):
                            return JsonResponse({"status": "error", "message": "Invalid reference data"}, status=400)
    ...
    def handle_ajax_request(self, request):
        ...
        if turtle_id:
            queryset = TrtTurtles.objects.select_related("species_code", "location_code", "turtle_status", "cause_of_death").filter(
                turtle_id=turtle_id
            )
        elif tag_id:
            turtle_ids = TrtTags.objects.filter(tag_id__icontains=tag_id).values_list("turtle_id", flat=True)
            queryset = TrtTurtles.objects.select_related("species_code", "location_code", "turtle_status", "cause_of_death").filter(
                turtle_id__in=turtle_ids
            )
        elif pit_tag_id:
            turtle_ids = TrtPitTags.objects.filter(pittag_id__icontains=pit_tag_id).values_list("turtle_id", flat=True)
            queryset = TrtTurtles.objects.select_related("species_code", "location_code", "turtle_status", "cause_of_death").filter(
                turtle_id__in=turtle_ids
            )
        elif other_id:
            turtle_ids = TrtIdentification.objects.filter(identifier__icontains=other_id).values_list("turtle_id", flat=True)
            queryset = TrtTurtles.objects.select_related("species_code", "location_code", "turtle_status", "cause_of_death").filter(
                turtle_id__in=turtle_ids
            )

        if queryset is None:
            return JsonResponse({"status": "error", "message": "No search criteria provided"})
        ...
        return JsonResponse({"status": "success", "data": turtle_data})
...
class FlipperTagsUpdateView(LoginRequiredMixin, SuperUserRequiredMixin, View):
    def post(self, request, *args, **kwargs):
        try:
            ...
            # Handle deletions
            if deleted_tags:
                TrtTags.objects.filter(tag_id__in=deleted_tags, turtle=turtle).update(
                    turtle=None, tag_status=TrtTagStatus.objects.get(tag_status="SAL")
                )
            ...
            return JsonResponse({"status": "success", "message": "Flipper tags updated successfully"})
...
class PitTagsUpdateView(LoginRequiredMixin, SuperUserRequiredMixin, View):
    def post(self, request, *args, **kwargs):
        try:
            ...
            # Handle deletions
            if deleted_tags:
                TrtPitTags.objects.filter(pit_tag_id__in=deleted_tags, turtle=turtle).update(
                    turtle=None, tag_status=TrtTagStatus.objects.get(tag_status="SAL")
                )
            ...
            for tag_data in pit_tags:
                ...
                tag, created = TrtPitTags.objects.get_or_create(
                    pit_tag_id=tag_id, defaults={"turtle": turtle, "tag_status": tag_status, "comments": tag_data.get("comments")}
                )
                if not created:
                    tag.turtle = turtle
                    tag.tag_status = tag_status
...
class IdentificationsUpdateView(LoginRequiredMixin, SuperUserRequiredMixin, View):
    def post(self, request, *args, **kwargs):
        try:
            ...
            # Handle deletions
            if deleted_identifications:
                TrtIdentification.objects.filter(turtle=turtle, identification_type__in=deleted_identifications).delete()
            ...
            for ident_data in identifications:
                ...
                TrtIdentification.objects.update_or_create(
                    ...
                    defaults={"identifier": identifier, "comments": ident_data.get("comments")},
                )
    ...
    def get_query_results(self, context):
        ...
        try:
            ...
            if context["data_type"] == "processed":
                ...
                query = TrtObservations.objects.filter(observation_date__gte=start_date, observation_date__lte=end_date)
                ...
                results = (
                    ...
                    .annotate(count=Count("turtle", distinct=True))
                    .order_by("place_code__place_code")
                )

            else:
                query = TrtDataEntry.objects.filter(observation_date__gte=start_date, observation_date__lte=end_date)
                ...
                has_turtle_results = has_turtle_query.values("place_code__place_code", "place_code__place_name", "turtle_id").distinct()
...
class BatchesReviewView(LoginRequiredMixin, SuperUserRequiredMixin, PaginateMixin, ListView):
    ...
    def get_queryset(self):
        ...
        queryset = queryset.filter(
            ...
            | (Q(error_message__isnull=False) & ~Q(error_message="None") & ~Q(error_message="Observation added to database"))
        )
...
class MergeTurtlesView(LoginRequiredMixin, View):
    ...
    def get_context_data(self):
        """Return the context data for template rendering"""
        return {"page_title": "Merge Turtles - " + settings.SITE_TITLE}
    ...
    def get_observations(self, turtle_id):
        ...
        observations = (
            TrtObservations.objects.filter(turtle_id=turtle_id)
            .select_related("turtle", "place_code")
            .values("observation_id", "observation_date", "turtle_id", "place_code", "comments")
            .order_by("-observation_date")
        )
        return list(observations)
    ...
    def post(self, request):
        ...
        if request.headers.get("X-Requested-With") == "FetchObservations":
            turtle_id = request.POST.get("turtle_id")
            if not turtle_id:
                return JsonResponse({"success": False, "error": "Turtle ID is required"})
        ...
        try:
            ...
            with connections["wamtram2"].cursor() as cursor:
                cursor.execute(
                    "EXEC dbo.MergeTurtlesWEB @SOURCE_TURTLE_ID = %s, @TARGET_TURTLE_ID = %s;", [source_turtle_id, target_turtle_id]
                )
